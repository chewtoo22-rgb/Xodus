...
def validate_manifest(project_dir: Path) -> dict:
    ...
    required = {
        "schema_version", "id", "name", "stage", "enabled_by_default",
        "boot_dependency", "login_dependency", "installer_dependency",
        "recovery_dependency", "network_required", "permissions",
    }
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(json.dumps({"schema_version": 1, "validated_projects": [m["id"] for m in manifests], "labs_isolated": True}, sort_keys=True))
    return 0
